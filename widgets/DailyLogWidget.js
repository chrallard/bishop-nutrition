...
export default class DailyLogWidget extends Component {
    ...
    setUid = async () => {//finds the user id to access the db
        let uid = await firebase.auth().currentUser.uid
        this.setState({ uid })
    }

    setUsersPlan = async () => {
        let usersPlan = await firebase.firestore().collection("userData").doc(this.state.uid).get().then((doc) => { return doc.data().plan }) //pulls the users plan from the db 
        this.setState({ usersPlan })
    }

    setPlanData = async () => {
        await firebase.firestore().collection("plans").doc(this.state.usersPlan).get().then((doc) => { // sets the correct portions from the db
